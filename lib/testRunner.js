...
export async function runOnce(dirName, options = {}, done) {
    ...
    let files = await glob(`${dirName}/**/*${options.fileSuffix}`);
    let filesFilter = options.filesFilter;
    if (filesFilter) {
        let filteredFiles = await filesFilter(files);
        return runNow(filteredFiles, options, done);
    }
    return runNow(files, options, done);
}

export function runOnceTeamcity(dirName, options = {}, done) {
    return runOnce(dirName, {
        ...options,
