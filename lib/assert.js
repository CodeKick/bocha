...
import { deepEqual, match as utilsMatch } from './utils.js';
...
assert.regexTest = function regexTest(actual, regex, message) {
    if (!regex.test(actual)) {
        let details = `Expected "${actual}" to match regex "${regex}"`;
        fail(actual, regex, combine({ message, details }), 'regexTest', assert.regexTest);
    }
};
