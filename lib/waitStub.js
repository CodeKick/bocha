    ...
    return function () {
        return new Promise(resolve => {
            if (stub.callCount >= targetCallCount) {
                ...
                stub.waitForCall(resolve, targetCallCount);
            }
        });
    };
}

function addWaitMethodToSinonSpy() {
    // stub() uses spy() so only needs to do this for spy

    let originalCreate = sinon.spy.create;
    sinon.spy.create = function () {
        let stub = originalCreate.apply(sinon.spy, Array.prototype.slice.call(arguments));
        let listenerMap = {};
