...
let Progress = require('./progress.cjs');
let escapeRe = require('escape-string-regexp').default;
...
let COLOR_TO_CLASS = {
    'diff added': 'errorDiff-added',
    'diff removed': 'errorDiff-removed',
};
...
function HTML(runner, options) {
    ...
    runner.on(EVENT_TEST_FAIL, function (test) {
        ...
        if (test.err.htmlMessage && stackString) {
            ...
            let diffHtml = '';
            try {
                let errCopy = { ...test.err };
                stringifyDiffObjs(errCopy);
                let diffString = unifiedDiffHtml(errCopy.actual, errCopy.expected);
                diffHtml = `<div style="color: var(--mocha-color)">${diffString}</div>`;
            }
            catch (err) {
                console.log('Failed to print diff');
                console.error(err);
            }
            el.appendChild(
                fragment('<pre class="error">%e%e%s</pre>', message, stackString, diffHtml)
            );
        }
...
function unifiedDiffHtml(actual, expected) {
    let indent = '      ';

    function cleanUp(line) {
        if (line[0] === '+') {
            return indent + colorLinesHtml('diff added', line);
        }
        if (line[0] === '-') {
            return indent + colorLinesHtml('diff removed', line);
        }
        if (line.match(/@@/)) {
            return '--';
        }
    ...
    return (
        ...
        ' ' +
        colorLinesHtml('diff removed', '- actual') +
        '\n\n' +
        lines.map(cleanUp).filter(notBlank).join('\n')
    );
}

function colorLinesHtml(name, str) {
    return str
        .split('\n')
        .map(str => colorHtml(name, str))
        .join('\n');
}

function colorHtml(type, str) {
    let className = COLOR_TO_CLASS[type];
    let strEncoded = htmlEncode(str);
    return `<span class="${className}">${strEncoded}</span>`;
}
