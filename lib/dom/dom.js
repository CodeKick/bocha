...
async function clickAndTick(selectorOrElement, time) {
    click(selectorOrElement);
    await timeoutWrapper(time);
}

async function setValueAndTick(selectorOrElement, value, time) {
    setValue(selectorOrElement, value);
    await timeoutWrapper(time);
}

// Focus will only work if the testDom is "visible" (not display none)
async function focusAndTick(selectorOrElement) {
    let element = getUniqueElement(selectorOrElement);
    element.focus();
    let event = document.createEvent('HTMLEvents');
...
async function blurAndTick(selectorOrElement) {
    let element = getUniqueElement(selectorOrElement);
    element.blur();
    let event = document.createEvent('HTMLEvents');
...
async function mouseupAndTick(selectorOrElement) {
    let element = getUniqueElement(selectorOrElement);
...
async function pasteIntoInputAndTick(text, selectorOrElement) {
    let element = getUniqueElement(selectorOrElement);
