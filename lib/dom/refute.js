...
refute.elementCount = function elementCount(selector, expected, message) {
    ...
    if (elementCount === expected) {
        ...
        fail(elementCount, expected, combine({ message, details }), 'NOT elementCount', refute.elementCount);
    }
};
...
refute.elementHasClass = function elementHasClass(selector, expected, message) {
    ...
    if (hasClass) {
        ...
        fail(selector, expected, combine({ message, details }), 'NOT hasClass', refute.elementHasClass);
    }
};
...
refute.elementIsChecked = function elementIsChecked(selector, message) {
    ...
    if (actualValue) {
        ...
        fail(actualValue, false, combine({ message, details }), 'NOT elementIsChecked', refute.elementIsChecked);
    }
};
...
refute.elementIsFocused = function elementIsFocused(selector, message) {
    ...
    if (isFocused) {
        ...
        fail(true, false, combine({ message, details }), 'NOT elementIsFocused', refute.elementIsFocused);
    }
};
